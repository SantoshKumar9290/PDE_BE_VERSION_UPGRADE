...
const engGenarateDocs = async (data) => {
    try {
        ...
        pdeStyle.content.push({ text: `DEED OF ${subDesc} OF TRUST`.toUpperCase(), alignment: 'center', bold: true, decoration: 'underline', margin: [10, y, 0, 0] });
        pdeStyle.content.push(' ');
        pdeStyle.content.push({ text: `This DEED OF${subDesc.toUpperCase()} OF TRUST is made and executed on ${data.executionDate}, by :`, style: ['p1sideHeaders'] });
        ...
        if (data?.executent && data?.executent.length > 0) {
            for (var i = 0; i < data.executent.length; i++) {
                ...
                if (data?.executent[i].partyType === "Public") {
                    ...
                    pdeStyle.content.push({
                        text:'(hereinafter called as SETTLOR which experssion) ',style:["p1Text"]
                    })
                } else {
                    pdeStyle.content.push({
        ...
        //             });
        //             pdeStyle.content.push({
        //                 text:'(Hereinafter called the “SECOND PARTY”  which expression shall wherever it occurs in this deed includes his heirs, executors, assignees and administrators).',style:["p1Text"]
        //             })
        //         } else {
        //             pdeStyle.content.push({
        //                 text: `M/s .${name}, ${address} is represented by`, style: ["p1Text"]
        ...
        pdeStyle.content.push({text:'\n IN WITNESS WHEREOF the said PARTNERS have hereto signed and executed this deed of dissolution. \n ',style:['p1Text']})
        ...
        //     }
        // }
        pdeStyle.content.push({ text: `Settlor :`, style: ['p1last_MRright'] })
...
const telGenarateDocs = async(data,path)=>{
    try{
        return new Promise( async (resolve, reject) => {
            ...
            pdfDocs.fontSize('18').text(`ట్రస్టు ${subDesc} దస్తావేజు`,40,y,{align:'center',underline:true});
            // pdfDocs.fontSize('13').text(`రూ ${data.amount} రూపాయలు విలువగల స్థిరాస్తి ${subDesc}.`,{align:'center'});
            pdfDocs.fontSize('13').text(`ఈ ట్రస్టు ${subDesc} దస్తావేజు ${yyyy} వ సంవత్సరం ${mm} నెల ${dd} వ తేదీన వ్రాయబడింది.`,{align:'center'});
            ...
            if(data.executent && data.executent.length >0){
                ...
                pdfDocs.fontSize(13).text('\n(అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “సెట్లరు“ గా  ఈ దస్తావేజునందు పరిగణించబడుదురు).',{align:'justify',lineGap:0});
                pdfDocs.moveDown();
                } 
            }else{
            ...
            //     pdfDocs.fontSize('13').text('\n (అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “2 వ పార్టీ“ గా ఈ దస్తావేజునందు పరిగణించబడుదురు).',{align:'justify',lineGap:0})
            //     pdfDocs.moveDown();
            //     }
            // }else{
            //     pdfDocs.fontSize('13').text(`శ్రీ/శ్రీమతి. .......,S/O. .........,వయస్సు ........ సంవత్సరాలు,ప్రస్తుతం......... లో నివాసము`,{align:'justify',lineGap:0});
            ...
            // pdfDocs.fontSize(13).text('\n ఈ క్రింది సాక్షుల సమక్షంలో పైన పేర్కొనిన తేదీన ఈ పార్టనర్షిప్ రద్దు దస్తావేజు పై  సంతకములు చేయడమైనది.\n',{align:'justify',lineGap:0})
            ...
            //     pdfDocs.moveDown();
            // }
            pdfDocs.fontSize('13').text(`సెట్లరు`,{align:'right',lineGap:0,underline:true});
