...
const engGenarateDocs = async (data) => {
    try {
        ...
        if (data?.executent && data?.executent.length > 0) {
            for (var i = 0; i < data.executent.length; i++) {
                ...
                if (data?.executent[i].partyType === "Public") {
                    ...
                    pdeStyle.content.push({
                        text:'(Hereinafter called the “SURETY” which expression shall wherever it occurs in this deed include her heirs, executors, assignees and administrators).',style:["p1Text"]
                    })
                } else {
                    pdeStyle.content.push({
        ...
        if (data?.claimant && data?.claimant.length > 0) {
            for (var i = 0; i < data.claimant.length; i++) {
                ...
                if (data?.claimant[i].partyType === "Public") {
                    ...
                    pdeStyle.content.push({
                        text:'(Hereinafter called the “ASSURED”  which expression shall wherever it occurs in this deed includes his heirs, executors, assignees and administrators).',style:["p1Text"]
                    })
                } else {
                    pdeStyle.content.push({
                        text: `M/s .${name}, ${address} is represented by`, style: ["p1Text"]
        ...
        pdeStyle.content.push({text:'\n IN WITNESS WHEREOF the surety  herein has set his/their/its hands and seal on the date, month and year above first written. \n ',style:['p1Text']})
    ...
    try{
        return new Promise( async (resolve, reject) => {
            ...
            if(data.executent && data.executent.length >0){
                ...
                pdfDocs.fontSize(13).text('\n(అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “హామీదారు“ గా ఈ దస్తావేజునందు పరిగణించబడుదురు).',{align:'justify',lineGap:0});
                pdfDocs.moveDown();
                } 
            }else{
            ...
            if(data.claimant && data.claimant.length >0){
                ...
                pdfDocs.fontSize('13').text('\n (అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “హామీ పొందువారు “ గా ఈ దస్తావేజునందు పరిగణించబడుదురు).',{align:'justify',lineGap:0})
                pdfDocs.moveDown();
                }
            }else{
                pdfDocs.fontSize('13').text(`శ్రీ/శ్రీమతి. .......,S/O. .........,వయస్సు ........ సంవత్సరాలు,ప్రస్తుతం......... లో నివాసము`,{align:'justify',lineGap:0});
            ...
            pdfDocs.fontSize(13).text('\nభద్రత  వివరాలు\n',{align:'justify',lineGap:0})
            pdfDocs.fontSize(13).text('\n ఈ క్రింది సాక్షుల సమక్షంలో పైన పేర్కొనిన తేదీన ఈ సెక్యూరిటీ బాండుపై హామీదారు సంతకములు చేయదమైనది.\n',{align:'justify',lineGap:0})
            pdfDocs.fontSize('13').text(`హామీదారు  `,{align:'left',lineGap:1});
            // if(data?.executent && data?.executent.length >0){
            //     let exNo =1
            //     for(let i of data.executent){
            ...
            //         pdfDocs.fontSize('12').text(`${exNo}`,{align:'right'});
                    
            //         pdfDocs.fontSize('12').text(`సంతకము :`+'\n'+`పేరు: ${toTitleCase(i.name)}`+'\n'+`ఆధార్ నెంబర్ : ${aadhar}`,{align:'right'});
            //         exNo = exNo+1;
                
            //     }
            
            // }else{
            //     pdfDocs.fontSize('13').text(`1. వ పార్టీ : .....................`,{align:'right',lineGap:0});
            //     pdfDocs.moveDown();
            // }
            ...
            pdfDocs.fontSize('13').text(`సాక్షులు`,{align:'left',lineGap:0,underline:true});
