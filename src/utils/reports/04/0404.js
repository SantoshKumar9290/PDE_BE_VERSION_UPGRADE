...
const engGenarateDocs = async (data, path) => {
  try {
    ...
    let englishHtmlFilePath = Path.join(__dirname,`../htmlFiles/04/04/english.hbs`);
...
const telGenarateDocs = async (data, path) => {
  try {
    let teluguHtmlFilePath = Path.join(__dirname,`../htmlFiles/04/04/telugu.hbs`);
...
const genarateDocsUsingHtml = async (data, path, htmlFilePath) => {
  try {
    ...
    if (data.claimant && data.claimant.length > 0) {
      ...
      let countClaimants = data.claimant.length 
        const numbers = [];
        for (let i = 1; i <= countClaimants; i++) {
          numbers.push(i);
        }
        data.claimant.countMembers = (numbers.join(', '));
    } else {
      data.claimant = [];
    }
    ...
    if (data.witness && data.witness.length > 0) {
      ...
      let countwitnesss = data.witness.length 
        const numbers = [];
        for (let i = 1; i <= countwitnesss; i++) {
          numbers.push(i);
        }
        data.witness.countMembers = (numbers.join(', '));
    } else {
      data.witness = [];
    }
