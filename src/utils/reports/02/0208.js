...
const engGenarateDocs = async (data,path)=>{
    try {
        STATIC_COVNENTS_COUNT = 2;
        console.log('PremKumar',data.executantDetails)
        let englishHtmlFilePath = Path.join(__dirname, `../htmlFiles/02/08/english.hbs`);
...
const telGenarateDocs = async (data, path) => {
    try {
        let teluguHtmlFilePath = Path.join(__dirname, `../htmlFiles/02/08/telugu.hbs`);
