...
hbs.registerHelper('wrapText', function (stringAddress) {
    return stringAddress.split(',').join(', ');
});

let STATIC_COVNENTS_COUNT = 4;

const engGenarateDocs = async (data,path)=>{
    try {
        STATIC_COVNENTS_COUNT = 4;
        console.log('PremKumar',data.executantDetails)
        let englishHtmlFilePath = Path.join(__dirname, `../htmlFiles/02/01/english.hbs`);
...
const telGenarateDocs = async (data, path) => {
    try {
        let teluguHtmlFilePath = Path.join(__dirname, `../htmlFiles/02/01/telugu.hbs`);
...
const imagePath = `${process.cwd()}/logos/ap_logo.jpg`;
const backgroundImage = `${process.cwd()}/logos/ap_logo.jpg`
const genarateDocsUsingHtml = async (data, path, htmlFilePath) => {
    try {
            ...
            const imageBuffer1 = fs.readFileSync(backgroundImage);
            ...
            if (data.witness && data.witness.length > 0) {
                data.witness.map((witnessDetails) => {
                    witnessDetails.index = count;
                    let add = witnessDetails.address.split("\n").join("")
                    add = add.split(" ").join("");
                    witnessDetails.stringAddress = add;
                    count = count + 1;
                })
            } else {
                data.witness = []
            ...
            if(data.payment && data.payment.length >0){	
                ...
                data.payment.map(async paymentDetails => {
                    ...
                    let paymentDateInFormat = new Date(paymentDetails?.dateOfPayment).toLocaleDateString();
            ...
            data.property?.map(propertyDetails => {
                ...
                if(propertyType == 'RURAL'){
                    ...
                    if(scProp == "FLAT "){
                        ...
                        propertyDetails.urbanBoundayDetails = {
                            east: propertyDetails.flatEastBoundry,
                            west: propertyDetails.flatWestBoundry,
                            south: propertyDetails.flatSouthBoundry,
                            north: propertyDetails.flatNorthBoundry
                        }
                        ...
                        console.log("NEW STRUCTURE DETAILS", newStructureDetails)
                        propertyDetails.newStructureDetails = newStructureDetails;
                        propertyDetails.floorPlinth = floorPlinth;
                        propertyDetails.cellarPlinth = cellarPlinth;
                        propertyDetails.urbanBoundayDetails = {
                            east: propertyDetails.eastBoundry,
                            west: propertyDetails.westBoundry,
                            south: propertyDetails.southBoundry,
                            north: propertyDetails.northBoundry
                        }
                    }else{
                        propertyDetails.isUrbanSite.push(1);
                    }
            ...
            let dynamicData = {
                ...
                chargableValue: totalMarketValue > landAmount ?  totalMarketValue: landAmount,
                executantDetails : data.executent,
                claimantDetails : data.claimant,
                witnessDetails: data.witness,
