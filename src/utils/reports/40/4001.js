...
const engGenarateDocs = async (data) => {
    try {
        ...
        if (data?.executent && data?.executent.length > 0) {
            for (var i = 0; i < data.executent.length; i++) {
                ...
                if (data?.executent[i].partyType === "Public") {
                    ...
                    pdeStyle.content.push({
                        text:'(Hereinafter called the “FIRST PARTY” which expression shall wherever it occurs in this deed include her heirs, executors, assignees and administrators).',style:["p1Text"]
                    })
                } else {
                    pdeStyle.content.push({
        ...
        //         if (data?.executent[i].partyType === "Public") {
        //             pdeStyle.content.push({
        //                 text: `\n Partners :  ${name}`, style:["p1last_MRright"]
        //             })
        //         } else {
        //             pdeStyle.content.push({
        //                 text: `\n Partners : ${name}`, style:["p1last_MRright"]
        ...
        if (data?.claimant && data?.claimant.length > 0) {
            for (var i = 0; i < data.claimant.length; i++) {
                ...
                if (data?.claimant[i].partyType === "Public") {
                    pdeStyle.content.push({
                        text: `\nSECOND PARTY : ${name}`, style: ["p4Text"]
                    })
                } else {
                    pdeStyle.content.push({
                        text: `\nSECOND PARTY : ${name}`, style: ["p4Text"]
...
const telGenarateDocs = async(data,path)=>{
    try{
        return new Promise( async (resolve, reject) => {
            ...
            pdfDocs.fontSize('18').text(`${subDesc} పత్రం`,40,y,{align:'center',underline:true});
            // pdfDocs.fontSize('13').text(`రూ ${data.amount} రూపాయలు విలువగల స్థిరాస్తి ${subDesc}.`,{align:'center'});
            pdfDocs.fontSize('13').text(`ఈ ${subDesc} పత్రం ${yyyy} వ సంవత్సరం ${mm} నెల ${dd} వ తేదీన వ్రాయబడింది.`,{align:'center'});
            ...
            if(data.executent && data.executent.length >0){
                ...
                pdfDocs.fontSize(13).text('\n(అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “1 వ పార్టీ“ గా  ఈ దస్తావేజునందు పరిగణించబడుదురు).',{align:'justify',lineGap:0});
                pdfDocs.moveDown();
                } 
            }else{
            ...
            pdfDocs.fontSize('13').text(`1 వ పార్టీ                             2 వ పార్టీ`,{align:'justify',lineGap:1});
