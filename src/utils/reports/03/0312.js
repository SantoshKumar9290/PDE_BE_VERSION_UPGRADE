...
const engGenarateDocs = async (data, path) => {
  ...
  //     pdeStyle.content.push({text:`GIFT DEED RESERVING LIFE INTEREST`.toUpperCase(),alignment: 'center',bold:true,decoration:'underline',margin:[10,y,0,0]});
  //     pdeStyle.content.push(' ');
  //     pdeStyle.content.push({text:`This GIFT DEED RESERVING LIFE INTEREST DEED is made and executed on ${data.executionDate}, by :`, style:['p1sideHeaders']});
  ...
  //                 })
  //                 pdeStyle.content.push({
  //                     text:`\n(Hereinafter referred to as  ‘the DONOR’, which expression shall, unless repugnant to the context, mean and include her legal heirs, assigns, executors and administrators)`
  //                 })
  ...
  //                 });
  //                 pdeStyle.content.push({
  //                     text:`\n(Hereinafter  referred to as ‘the DONEE’, which expression shall, unless repugnant to the context, mean and include her legal heirs, assigns, executors and administrators)`
  //                  });
  //             }else{
  //                 pdeStyle.content.push({
  //                     text:`M/s .${name}, ${address} is represented by`,style:["p1Text"]
...
const telGenarateDocs = async (data, path) => {
  ...
  //             pdfDocs.moveDown();
  //         }
  //         pdfDocs.fontSize('13').text(`(అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “దాన స్వీకర్తలు”గా ఈ దస్తావేజునందు పరిగణించబడుదురు).`,{align:'justify',lineGap:0})
  //         if(data.covanants?.acquireCovenents && data.covanants?.acquireCovenents.length >0){
  //             let aqCovanents=[]
  ...
  //         pdfDocs.fontSize('13').text(`దాన స్వీకర్తల యందు దాతలకు గల సహజ ప్రేమాభిమానముల చేతనూ, దాన స్వీకర్తలకు కొంత స్థిరోపాధి కలిపించే ఉద్దేశ్యము చేత, దాన స్వీకర్త దాత పై ఆధారపడి యుండుట చేతనూ, ప్రేమయే ప్రతిఫలంగా యెంచి ఉచితముగా షెడ్యూలు ఆస్థిని దఖలు పర్చడమైనది. షెడ్యూల్ ఆస్థి నందు జీవితకాలం అనుభవించు హక్కులను దాతలు కలిగిఉందురు. దాతల తదనంతరం దాన స్వీకర్తలు షెడ్యూల్ ఆస్థి పై పూర్తి హక్కులను పొంది సదరు ఆస్థిని దాన స్వీకర్తలు మునిసిపల్, యితర ప్రభుత్వ లెక్కలలో వారి పేరున నమోదు చేయించుకొని, సదరాస్థికి చెల్లించవలసిన పన్నులు వగైరాలు చెల్లించుకుంటూ, వారి పుత్ర, పౌత్ర వంశపారంపర్యంగా ఋణ, దాన విక్రయాధికారపు సమస్త సంపూర్ణ హక్కులతోను, యీజిమెంటు రహదారి హక్కులతోను, దాన స్వీకర్త యిష్టానుసారం శాశ్వతముగా, స్వేచ్ఛగా, సుఖాన అనుభవించవలెను. `,{align:'justify',lineGap:1});
  ...
  //         pdfDocs.fontSize('13').text(`\n దాతలు:`,{align:'left',lineGap:0,underline:true});
