...
//         pdeStyle.content.push({ text: `This ${subDesc.toUpperCase()} is executed on this  ${data.executionDate}, by :`, style: ['p1sideHeaders'] });
...
//         pdeStyle.content.push({
//             text:"NOW THIS AGREEMENT WITNESSETH AS FOLLOWS:",style:["p5Text"]
//         })
...
//         let sNo = 1
//         let cvStaticArry = [];
//         covanantsData.Agreement.map((cv) => {
...
//             pdfDocs.fontSize(13).text('\n ప్రస్థుతము ఈ అగ్రీమెంటు ఈక్రింది విషయాలను దృవీకరించుచున్నది \n',{align:"justify",lineGap:0})
//             if(data.covanants?.acquireCovenents && data.covanants?.acquireCovenents.length >0){
//                 let aqCovanents=[]
...
const telGenarateDocs = async (data, path) => {
  try {
    let teluguHtmlFilePath = Path.join(__dirname, `../htmlFiles/34/telugu.hbs`);
