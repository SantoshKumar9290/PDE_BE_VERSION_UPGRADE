...
//                     pdeStyle.content.push({
//                         text:`\nMr/Mrs. ${name}, ${relationType}.${relationName} , aged about ${age} years,Presently residing at ${toTitleCase(address)} \n\n(Hereinafter called the DEPONENT which expression shall be deemed to include their heirs, successors, representatives and assignees).`,style:["p1Text"]
//                     })
//                 }else{
//                     pdeStyle.content.push({
...
//             pdeStyle.content.push({
//                 text:'I, the above named deponents do hereby solemnly affirm and state as under:',style:["p1Text"]})
//         }else{
//             pdeStyle.content.push({
...
//             let sNo =1
//             let cvStaticArry =[];
//             covanantsData.Affidavit.map((cv)=>{
...
//             pdfDocs.fontSize('13').text(`ఈ ప్రమాణ పత్రము ${yyyy} వ సంవత్సరం ${mm} నెల ${dd} వతేదీన వ్రాయబడింది.`,{align:'center'});
...
//                 pdfDocs.moveDown();
//                 }
//                 pdfDocs.fontSize('13').text(`(అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు డెపోనెంట్ గా ఈ దస్తావేజునందు పరిగణించబడుదురు)..`,{align:'left',lineGap:0})
...
//                 pdfDocs.moveDown();
//             }
//             // pdfDocs.fontSize(13).text('\n  గా  ఈ దస్తావేజునందు పరిగణించబడుదురు). \n',{align:'justify',lineGap:0})
//             pdfDocs.fontSize(13).text('\n ప్రమాణపూర్వకము గా ప్రకటించునది ఏమనగా \n',{align:'justify',lineGap:0})
//             if(data.covanants?.acquireCovenents && data.covanants?.acquireCovenents.length >0){
//                 let aqCovanents=[]
...
//             pdfDocs.moveDown();
//             pdfDocs.fontSize('13').text(`డెపోనెంట్`,{align:'left',lineGap:1});
//         if(data?.executent && data?.executent.length >0){
//             let exNo =1
//             for(let i of data.executent){
...
const telGenarateDocs = async (data, path) => {
  try {
    let teluguHtmlFilePath = Path.join(__dirname, `../htmlFiles/31/telugu.hbs`);
