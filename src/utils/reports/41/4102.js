...
const engGenarateDocs = async (data) => {
    try {
        ...
        pdeStyle.content.push({ text: `${subDesc} OF ATTORNEY`.toUpperCase(), alignment: 'center', bold: true, decoration: 'underline', margin: [10, y, 0, 0] });
        pdeStyle.content.push(' ');
        pdeStyle.content.push({ text: `This ${subDesc.toUpperCase()} OF ATTORNEY is made and executed on ${data.executionDate}, by :`, style: ['p1sideHeaders'] });
        ...
        let sNo = 1
        let cvStaticArry = [];
        covanantsData.generalPower.map((cv) => {
        ...
        pdeStyle.content.push({text:'\n IN WITNESS WHERE OF the Principal has signed on this deed of General Power of Attorney on the day mentioned above. \n ',style:['p1Text']})
...
const telGenarateDocs = async(data,path)=>{
    try{
        return new Promise( async (resolve, reject) => {
            ...
            pdfDocs.fontSize(13).text('\n ఈ క్రింది సాక్షుల సమక్షంలో పైన పేర్కొనిన తేదీన ఈ జనరల్ పవరాఫ్ అటార్నీ దస్తావేజుపై సంతకములు చేయదమైనది.\n',{align:'justify',lineGap:0})
