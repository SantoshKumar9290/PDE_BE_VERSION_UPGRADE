...
const engGenarateDocs = async (data, path) => {
  try {
    ...
    //     pdeStyle.content.push({text:`\n\nAND WHEREAS ALL MONEYS due and owing to the security of the PRINCIPAL INDENTURE have been fully paid and satisfied and the Mortgagee has accordingly at the request of the Mortgagor agreed to execute this Re-Conveyance of the Mortgaged premises as is hereinafter contained.`,style:["p1Text"]})
    //     // pdeStyle.content.push({
...
const telGenarateDocs = async (data, path) => {
  try {
    ...
    //     pdfDocs.fontSize('13').text(`యిందుకు  సాక్షులు :\n\n`,{align:'left',lineGap:0,underline:true});
