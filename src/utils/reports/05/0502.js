...
const engGenarateDocs = async (data, path) => {
  try {
    ...
    // pdeStyle.content.push({ text: `RELEASE DEED`.toUpperCase(), alignment: 'center', bold: true, decoration: 'underline', margin: [10, y, 0, 0] });
    // pdeStyle.content.push(' ');
    // pdeStyle.content.push({ text: `This RELEASE DEED is made and executed on this ${dd} - ${mm} - ${yyyy}, by :`, style: ['p1sideHeaders'] });
    ...
    //     pdeStyle.content.push({ text: `NOW THIS DEED OF RELINQUISHMENT WITNESSETH AS FOLLOWS :`, align: 'left', bold: true })
    //     let sNo = 1
    //     let cvStaticArry = [];
    ...
    // })
    // pdeStyle.content.push({
    //     text: 'The Schedule mentioned property is not a Government Land nor an Assigned land nor a property belonging to Wakf Board nor a property belonging to Endowments Department.'
    //     , style: '[p1Text_MR]'
    // })
...
const telGenarateDocs = async (data, path) => {
  try {
    ...
    //     pdfDocs.fontSize('18').text(`హక్కు విడుదల దస్తావేజు`, 40, y, { align: 'center', underline: true });
    //     // pdfDocs.fontSize('13').text(`రూ ${data.amount} రూపాయలు విలువగల స్థిరాస్తి ${subDesc}.`,{align:'center'});
    //     pdfDocs.fontSize('13').text(`ఈ హక్కు విడుదల దస్తావేజు ${yyyy} వ సంవత్సరం ${mm} నెల ${dd} వ తేదీన వ్రాయబడింది.`, { align: 'center' });
    ...
    //             pdfDocs.moveDown();
    //         }
    //         pdfDocs.fontSize('13').text(`అనువారు, వారి వారసులు, స్తానీయులు, రెప్రెసెంటేటివ్ లు, ఏజెంట్లు మరియు అసైనీలు “రిలీసర్ “ గా ఈ దస్తావేజునందు పరిగణించబడుదురు`, { align: 'left', lineGap: 0 });
    //         pdfDocs.moveDown()
    //     } else {
