...
const { InsertWordOnCerIntervel } = require("../../helper");
...
const engGenarateDocs = async (data, path) => {
  try {
    ...
    //     pdeStyle.content.push({text:`This ${subDesc.toUpperCase()} DEED is made and executed on this ${dd} - ${mm} - ${yyyy}, by :`, style:['p1sideHeaders']});
    ...
    //     pdeStyle.content.push({text:`\nIn witness whereof the Releasor  have signed on this receipt on the day, month & year mentioned above.`,style:['p1Texr_MR']})
...
const telGenarateDocs = async (data, path) => {
  try {
    ...
    //     pdfDocs.fontSize('18').text(`ఫెడరేషన్ లేదా సొసైటీ మరియు సొసైటీ మధ్య జరిగే హక్కు విడుదల`,40,y,{align:'center',underline:true});
    //     // pdfDocs.fontSize('13').text(`రూ ${data.amount} రూపాయలు విలువగల స్థిరాస్తి ${subDesc}.`,{align:'center'});
    //     pdfDocs.fontSize('13').text(`ఈ హక్కు విడుదల ${yyyy} వ సంవత్సరం ${mm} నెల ${dd} వ తేదీన వ్రాయబడింది.`,{align:'center'});
    ...
    //     pdfDocs.fontSize('13').text(`అయితే సదరు దస్తావేజు తాలూకా వృద్దులు చెల్లిపోగా నిలిచియున్న అసలు సొమ్ము రిలీజీ నగదు రూపములో యీ దిగువ సాక్షులు సమక్షములో రిలీజర్ కు యిచ్చినందున సొమ్ము పూర్తిగా ముట్టినది. గాన యీ రీకన్వేయన్స్ డీడును వ్రాయించి యివ్వడమైనది.`,{align:'justify',lineGap:0})
    //     pdfDocs.moveDown();
